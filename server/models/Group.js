...
const CreateGroup = async (groupName,Email) => {
    return await pool.query('with first_insert as ( insert into "group"("groupName") values($1) RETURNING id, "groupName"), second_insert as (select id from "user" where "Email"=$2) insert into "Role"("groupId","userId","is_admin","is_user")  values ( (select id from first_insert), (select id from second_insert),'+'true'+','+'false'+') Returning (select "groupName" from first_insert)',[groupName,Email])
}

const AskToJoinGroup = async(name, groupName) =>{
    return await pool.query('with first_insert as(select id from "user" where "Name"=$1),second_insert as(select id,"groupName" from "group" where "groupName"=$2),third_insert as(insert into "Role" ("groupId","userId","is_admin","is_user") values ((select id from second_insert),(select id from first_insert),false,false))select "groupName" from "group" where "groupName"=(select "groupName" from second_insert)',[name,groupName])
}
