...
const postRegistration = async (req, res, next) => {
    ...
    try {
        if (!req.body.Name || req.body.Name.length === 0) return next(new ApiError('Invalid name for user', 400));
...
const postLogin = async (req, res, next) => {
    ...
    try {
        ...
        return res.status(200).json(createUserObject(user.Name, user.id, user.Email, user.CreationDate, token));
    } catch (error) {
        return next(error);
    };
};
